...
const handler = async (req, res) => {
  try {
    ...
    `)
      ...
      const arrayFile = [];
      let dir =
        process.env.NEXT_APP_JDIH_PATH + "uploads/" + result[0].idperaturan;
      ...
      res.end(
        JSON.stringify({ row: result, file: arrayFile, v: arrayFile.length })
      );
    ...
    `)
      ...
      questionMarks = questionMarks.substr(0, questionMarks.length - 2);
      if (rows.length > 0) {
        const insert = await req.db.raw(
          "INSERT INTO download_view (`iddownloadview`, `idperaturan`, `browser`, `os`, `ip`, `wilayah`, `negara`, `saat_ini`, `keterangan`, `tanggal`, `front_back`, `user`) VALUES " +
            questionMarks +
            " ON DUPLICATE KEY UPDATE saat_ini = VALUES(`saat_ini`)",
          values
        );
        if (insert[0].affectedRows > 0) {
          ...
          if (query.length > 0) {
            arrayData.push({
              id: query[0].idperaturan,
              online: singkatNumber(query[0].online),
              lihat: singkatNumber(query[0].lihat),
              unduh: singkatNumber(query[0].unduh),
              suka: singkatNumber(query[0].suka),
              tidak: singkatNumber(query[0].tidak),
            });
          }
        }
      }
      ...
      const resx = req.db
        ...
        .timeout(1000, { cancel: true });
      const arrIn = [];
      const resultData = await resx
        .clone()
        .whereIn("idperaturan", req.body.idperaturan);
      for (let item_p of resultData) {
        arrIn.push({
          id: item_p.idperaturan,
      ...
      arrayData = [
        {
          onlinehit: singkatNumber(resultDataHit[0].Online),
          lihathit: singkatNumber(resultDataHit[0].Lihat),
          unduhhit: singkatNumber(resultDataHit[0].Unduh),
          sukahit: singkatNumber(resultDataHit[0].Suka),
          tidakhit: singkatNumber(resultDataHit[0].Tidak),
        },
      ];
      res.status(200);
      res.setHeader("Content-Type", "application/json");
