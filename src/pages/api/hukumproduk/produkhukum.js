  ...
  try {
    if (req.method === 'POST') {
      ...
      if (keywords.length > 1){
        ...
        namajenisSQL.push(`if (b.nama_jenis = '${titleCase(escQuery)}', ${scoreFullNamaJenis}, 0)`);
        nomorSQL.push(`if (ABS(a.no_peraturan) = '${escQuery}', ${scoreFullNomor}, 0)`);
        tahunSQL.push(`if (a.tahun = '${escQuery}', ${scoreFullTahun}, 0)`);
        tentangSQL.push(`if (a.tentang = '${escQuery}', ${scoreFullTentang}, 0)`);
      }
  
      for (const key of keywords){
        let item = escapeHtml(key)
        jenisSQL.push(`if (b.jns = '${item}', ${scoreJenisKeyword}, 0)`);
        namajenisSQL.push(`if (b.nama_jenis = '${titleCase(item)}', ${scoreNamaJenisKeyword}, 0)`);
        nomorSQL.push(`if (ABS(a.no_peraturan) = '${item}', ${scoreNomorKeyword}, 0)`);
        tahunSQL.push(`if (a.tahun = '${item}', ${scoreTahunKeyword}, 0)`);
        tentangSQL.push(`if (a.tentang = '${item}', ${scoreTentangKeyword}, 0)`);
      }
      ...
      sql_x = `, ((`+jenisSQL.join(' + ')+`) + (`+namajenisSQL.join(' + ')+`) + (`+nomorSQL.join(' + ')+`) + (`+tahunSQL.join(' + ')+`) + (`+tentangSQL.join(' + ')+`)) as relevance`;
    } else {
      sql_x = '';
    }
    ...
    const resulData = req.db
     .select('a.idperaturan', 'a.no_peraturan', 'a.tahun', 'a.tentang', 'b.jns', 'b.nama_jenis',
      req.db.raw(`SUM(CASE WHEN c.keterangan='lihat' AND c.saat_ini = '1' THEN 1 ELSE 0 END) AS Online,
      ...
      .whereRaw(thn)
      .whereRaw(status)
      .groupBy('a.idperaturan', 'a.idperaturan', 'a.no_peraturan', 'a.tahun', 'a.tentang', 'b.jns', 'b.nama_jenis')
      ...
      if(check1.length > 1) {
        ...
        if(cariPch !== "") {
          ...
          for (const key of keywords){
            let item = escapeHtml(key)
            tentangSQL.push(`if (a.tentang LIKE '%${item}%', ${scoreTentangKeyword}, 0)`);
          }
          if (tentangSQL.length === 0){
              tentangSQL.push(0);
          }
...
function newFunctionResult(result, arrayData, arrayData_xid, arrayData_xxid) {
  ...
  for (let item_p of result) {
    ...
    arrayData_xid.push({ id: ixx, online: singkatNumber(item_p.Online), lihat: singkatNumber(item_p.Lihat), unduh: singkatNumber(item_p.Unduh), suka: singkatNumber(item_p.Suka), tidak: singkatNumber(item_p.Tidak) });
...
export function filterSearchKeys(qu) {
  ...
  let words = [];
  let list = ["di","itu","sebuah","dan","dari","atau","saya","kamu","dia","kami","mereka","untuk","tapi","bahwa","ini","kemudian","/"];
  let c = 0;
  let pch = match.split(" ");
