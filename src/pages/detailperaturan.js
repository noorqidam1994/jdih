...
function Dateailperaturan({ data }) {
    ...
    useEffect(() => {
      const unsubscribe = onSnapshot(q, async (colSnapshot) => {
        ...
        if (newTasks.length > 0 && newTasks !== undefined) {
          ...
          const res_p = await axiosInstance
            ...
            .then((response) => {
              ...
              if (json.row !== undefined) {
                ...
                setStatusOnline(res);
                $(".alertcustom").hide().show("slow").delay(3e3).hide("slow");
              }
            });
        }
      });
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < label.length; i++) {
      ...
      let textx = label[i].innerText.split(" ");
      let nthn = textx[1].split("/");
    ...
    if (response.status === 200 && intiData.row[0] !== undefined) {
      ...
      if (i_bwh.length > 0) {
        i_bwh.map((isi, i) => {
          ...
          let pchComma = i_ats[i].split("<label");
          let loopArrayIsi = "";
          pchComma.map((xx, xi) => {
            if (xi > 0) {
              let jd = "<label" + xx;
              ...
              <div className="modal-c-tabs">
                ...
                <div className="modal-body mb-1 liak_scroll pl-2 pr-2 pt-0">
                  ...
                  <div className="col-xl-12 col-md-12 text-right">
                    <button
                      className="blue-gradient btn btn-md validBtn"
