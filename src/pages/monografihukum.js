...
const Monografihukum = ({ data }) => {
  ...
  function clickDetailEvent(j, t, jdl) {
    let judulj = jdl.split(' ').join('+');
    let judul2 = judulj.split('/').join('_');
    let noSlashj = j.split(' ').join('+');
    router.push('/detailmonografi/'+noSlashj+'/'+judul2+'/'+t, undefined, { shallow: true })
  }
  ...
  const handleClickbtnSearch = csp => {
    ...
    router.push('/monografi-hukum/'+lctx[1]+'/'+encodeURI(link), undefined, { shallow: true })
  }
  ...
  const handleChangetahun = async (x) => {
    ...
    if (jsonD.result.length > 0) {
      $.each(jsonD.result, function(i, v) {
        let chk_c = $("#listThn").find("input[type=checkbox][value='"+v.tahun_terbit+"']").is(':checked');
        array.push(<label className="option_item" key={v.tahun_terbit} id={v.tahun_terbit} onChange={handleClickthnCari}>
          <input type="checkbox" className="checkbox" name="thn_x" value={v.tahun_terbit} defaultChecked={chk_c ? true : false} />
          <div className="option_inner _jdih">
          <div className="tickmark"></div>
          <div className="name glrthn">{v.tahun_terbit}</div>
          </div>
          </label>);
      });
  ...
  const hendleBtnTerapkan = trapkan => {
    ...
    router.push('/monografi-hukum/'+lctx[1] + akuksearch, undefined, { shallow: true })
  }
  ...
  return (
    <Page title="Monografi Hukum" description="Monografi Hukum">
    <div className="loading" style={{display: 'none'}}>Loading...</div>
      ...
      <div className="search-box sepok_ats" style={{padding: '0 3.6rem 0 1rem !important'}}>
      ...
      {undefined !== dvaluedata && 0 < dvaluedata.length ? dvaluedata.map( (d, k) => {
      return (
      <section className="dataLoad col-12 col-md-12 mt-3" key={d.idmonografi} id={d.idmonografi}>
      ...
      <div className="caption" onClick={() => clickDetailEvent(d.jenis_monografi, d.tahun_terbit, d.judul)}>
      <p className="group inner list-group-item-text pstyle d1"style={{fontSize: "large"}}>{d.judul.toUpperCase()}</p>
      <p className="group inner list-group-item-text d4">{d.teu}</p>
      ...
      <div className="filter-jdih-header" id="h_jenis">Jenis Monografi</div>	
      <div className="container pl-0 pr-0" id="listJns">
      {djenis && djenis.map(item => (
        <label className="option_item" key={item.jenis_monografi} id={item.jenis_monografi}>
        <input type="checkbox" className="checkbox chckjenis" name="jns[]" value={item.jenis_monografi} />
        <div className="option_inner _jdih">
        <div className="tickmark"></div>
        <div className="name glrjns">{item.jenis_monografi}</div>
        </div>
        </label>
        ))}
      </div>
      <div style={{clear:'both'}}></div>
      <div className="filter-jdih-header" id="h_tahun">Tahun Terbit</div>
      ...
      {dtahun && dtahun.map(item => (
        <label className="option_item" key={item.tahun_terbit} id={item.tahun_terbit}>
        <input type="checkbox" className="checkbox chcktahun" name="thn[]" value={item.tahun_terbit} />
        <div className="option_inner _jdih">
        <div className="tickmark"></div>
        <div className="name glrthn">{item.tahun_terbit}</div>
        </div>
        </label>
      ))}
      </div>
...
export async function getJenis() {
  const response = await axiosInstance.get('/api/monografi/jns');
...
export async function getTahun(Lmt) {
  const response = await axiosInstance.get('/api/monografi/thn?lmt='+Lmt)
  if(response.status === 500) {
    return { result: {data: '', jml: ''}}
  } else {
...
export async function cariTahun(qu) {
  const response = await axiosInstance.get('/api/monografi/thncari?vl='+qu)
  if(response.status === 500) {
    return { result: []}
  } else {
    return { result: await response.data}
...
export async function getValuedata(srh, jns, thn, crnPg, lmtDt) {
    ...
    const res_p = await axiosInstance.post('/api/monografi/apidata', resOpt);
    if(res_p.status === 500) {
      return { result: {data: '', jml: ''}}
    } else {
...
export async function getServerSideProps(context) {
  ...
  let jnsxx, thnxx, jnsxxPush = [], thnxxPush = [], jns_unyinx = [], thn_unyinx = [], totalDjenis, totalDtahun, pagexx;
  if (context.req.headers.cookie !== undefined) {
    ...
    if (prse.thn_x !== '' && prse.thn_x !== undefined) {
      ...
      for (let ju = 0; ju < pchx_j.length; ju++) {
        jns_unyinx.push({jenis_monografi: pchx_j[ju]});
      }
    ...
    if (prse.thn_unyin !== '' && prse.thn_unyin !== undefined) {
      ...
      for (let jt = 0; jt < pchx_t.length; jt++) {
        thn_unyinx.push({tahun_terbit: pchx_t[jt]});
      }
  ...
  return { props: { data: { srchx, valuedata, dataJenis, jml_j, dataTahun, jml_t, jnsxx, thnxx, pagexx  } } };
}

export default Monografihukum