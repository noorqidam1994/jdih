...
const Artikelhukum = ({ data }) => {
  ...
  function clickDetailEvent(j, t, jdl) {
    ...
    router.push(
      "/artikeldetail/" + noSlashj + "/" + judul2 + "/" + t,
      undefined,
      { shallow: true }
    );
  }
  ...
  const handleClickbtnSearch = (csp) => {
    ...
    router.push("/artikel/" + lctx[1] + "/" + encodeURI(link), undefined, {
      shallow: true,
    });
  };
  ...
  const handleChangetahun = async (x) => {
    ...
    console.log(jsonD.result);
    if (jsonD.result.length > 0) {
      $.each(jsonD.result, function (i, v) {
        let chk_c = $("#listThn")
          .find("input[type=checkbox][value='" + v.tahun_terbit + "']")
          .is(":checked");
        array.push(
          <label
  ...
  const hendleBtnTerapkan = (trapkan) => {
    ...
    router.push("/artikel/" + lctx[1] + akuksearch, undefined, {
      shallow: true,
    });
  };
              ...
              <div className="container pl-0 pr-0" id="listJns">
                {djenis &&
                  djenis.map((item) => (
                    <label
                      className="option_item"
                      key={item.jenis_artikel}
                      id={item.jenis_artikel}
                    >
                      <input
                        type="checkbox"
                        className="checkbox chckjenis"
                        name="jns[]"
                        value={item.jenis_artikel}
                      />
                      <div className="option_inner _jdih">
                        <div className="tickmark"></div>
                        <div className="name glrjns">{item.jenis_artikel}</div>
                      </div>
                    </label>
                  ))}
              </div>
...
export async function getJenis() {
  const response = await axiosInstance.get("/api/artikelhukum/jns");
...
export async function getTahun(Lmt) {
  const response = await axiosInstance.get(
    "/api/artikelhukum/tahun?lmt=" + Lmt
  );
  if (response.status === 500) {
    return { result: { data: "", jml: "" } };
  } else {
...
export async function cariTahun(qu) {
  const response = await axiosInstance.get(
    "/api/artikelhukum/thncari?vl=" + qu
  );
  if (response.status === 500) {
    return { result: [] };
  } else {
  ...
  if (context.req.headers.cookie !== undefined) {
    ...
    if (prse.jns_unyin !== "" && prse.jns_unyin !== undefined) {
      ...
      for (let ju = 0; ju < pchx_j.length; ju++) {
        jns_unyinx.push({ jenis_artikel: pchx_j[ju] });
      }
    } else {
      jns_unyinx.push("");
    }
